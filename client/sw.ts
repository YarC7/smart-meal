...
const logsQueue = new BackgroundSyncPlugin('logs-queue', { maxRetentionTime: 24 * 60 });
const groceryQueue = new BackgroundSyncPlugin('grocery-queue', { maxRetentionTime: 24 * 60 });

registerRoute(
  ({ url, request }) => url.pathname.startsWith('/api/logs') && ['POST','PUT','PATCH'].includes(request.method),
  new NetworkOnly({ plugins: [logsQueue] }),
  'POST'
);
registerRoute(
  ({ url, request }) => url.pathname.startsWith('/api/grocery') && ['POST','PUT','PATCH'].includes(request.method),
